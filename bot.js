/*  ────────────────────────────────────────────────────────────────────
    ...
    – de-duping, event support, RuneLite *and* Dink webhooks
    – handles BOTH structured JSON and plain-text loot messages
    ──────────────────────────────────────────────────────────────────── */
...
const DEDUP_WINDOW_MS = 10_000;                         // 10 s anti-spam

/* ── express ───────────────────────────────────────────────────────── */
const app = express();
app.use(bodyJson({ limit: "200kb" }));                  // accept JSON bodies
...
const seenRecently = new Map();         // key → lastSeenTime(ms)
setInterval(() => {
  const now = Date.now();
...
async function commitToGitHub() {
  if (!GITHUB_PAT) return;                           // no PAT = skip
  const git = simpleGit();
  await git.add(".");
...
/* ── ----------------------------------------------------------------
   GENERIC *loot* handler – called from both /logLoot and /dink routes
   ---------------------------------------------------------------- */
async function handleLoot(killer, victim, gp, raw, res) {
  if (
    clanOnlyMode &&
  ...
  // de-dupe on the full raw line (or generated key) so we don't spam
  const dupKey = `L|${raw}`;
...
/* -------- /logKill (direct HTTP from your RuneLite plugin) -------- */
...
/* -------- /logLoot (plain JSON from your RuneLite plugin) ---------- */
app.post("/logLoot", (req, res) => {
  ...
  return handleLoot(killer, victim, gp, lootMessage.trim(), res);
});

/* -------- /dink (DinkPlugin webhook) ------------------------------- */
app.post("/dink", (req, res) => {
  const p = req.body;

  /* 1️⃣ Structured JSON from newer Dink versions --------------- */
  if (
    p?.type === "CHAT" &&
    ...
    const killer = p.extra.username;
    const victim = p.extra.target;
    const gp     = p.extra.amount;
    const rawKey = `STRUCT|${killer}|${victim}|${gp}`;
    return handleLoot(killer, victim, gp, rawKey, res);
  }

  /* 2️⃣ Fall back to parsing the plain message ------------------ */
  if (
    p?.type === "CHAT" &&
    p?.extra?.type === "CLAN_CHAT" &&
    ...
    if (!m) return res.status(204).end();      // not a loot line – ignore
    const [, killer, victim, gpStr] = m;
    const gp = Number(gpStr.replace(/,/g, ""));
    return handleLoot(killer, victim, gp, msg.trim(), res);
  }

  /* 3️⃣ Anything else – ignore */
  return res.status(204).end();
});

/* ── Boot Express only after Discord login ───────────────────────── */
client.once("ready", () => {
...
client.on(Events.MessageCreate, async msg => {
  ...
  /* !hiscores ---------------------------------------------------- */
  if (text === "!hiscores") {
    const board = Object.entries(chatKillCounts)
      .map(([n, k]) => {
  ...
  if (text === "!finishevent") {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `Saved as \`${file}\` and switched back to **default**.`
      )
      .setColor(0xFF0000)
      .setTimestamp();
    return msg.channel.send({ embeds: [embed] });
